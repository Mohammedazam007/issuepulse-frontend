...
function ImageLightbox({ url, onClose }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 animate-fade-in" onClick={onClose}>
      ...
      <img src={url} alt="Full size" onClick={e => e.stopPropagation()} className="max-w-[92vw] max-h-[88vh] object-contain rounded-xl shadow-2xl" />
    </div>
  )
}
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto">
       <div className="flex min-h-full items-start justify-center p-4 pt-10">
          <div className="bg-white rounded-2xl shadow-modal w-full max-w-lg p-6 animate-fade-up" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              {/* Issue photo */}
              {complaint.imageUrl && (
                <div className="relative group cursor-zoom-in" onClick={() => setLightboxUrl(`${BASE_URL}${complaint.imageUrl}`)}>
                  <img src={`${BASE_URL}${complaint.imageUrl}`} alt="Issue"
              ...
              {complaint.flaggedAsFraud && (
                <div className="flex items-center gap-2.5 bg-orange-50 border border-orange-200 text-orange-800 rounded-xl p-3.5 text-sm">
                  ...
                  <div><span className="font-semibold">Fraud Flag: </span>{complaint.fraudReason}</div>
                </div>
              )}
              ...
              <div>
                ...
                <textarea rows={3} value={message} onChange={e => setMessage(e.target.value)}
                  placeholder="Write a resolution or update message…" className="input-field resize-none" />
              </div>
              ...
              <div>
                ...
                <input type="file" accept="image/*" onChange={e => setImage(e.target.files[0])}
                  className="block w-full text-sm text-slate-500 file:mr-3 file:py-2 file:px-3 file:rounded-xl file:border file:border-slate-200 file:text-sm file:font-semibold file:bg-white file:text-slate-700 hover:file:bg-slate-50 file:transition-all cursor-pointer" />
              </div>
            </div>
      ...
      <div className="card mb-6">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative">
            ...
            <input placeholder="Search title or student…" value={search} onChange={e => setSearch(e.target.value)} className="input-field pl-10 w-52" />
          </div>
            ...
            {['ALL', ...STATUSES].map(s => (
              <button key={s} onClick={() => setFS(s)}
                className={`text-xs px-3 py-1.5 rounded-lg border font-semibold transition-all ${
                  filterStatus === s ? 'bg-primary text-white border-primary shadow-glow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-primary/40 hover:text-primary'}`}>
                {s === 'ALL' ? 'All' : STATUS_LABELS[s] || s}
              </button>
            ))}
          </div>
          {categories.length > 0 && (
            <select value={filterCategory} onChange={e => setFC(e.target.value)} className="input-field w-auto text-xs py-2">
          ...
          <button onClick={() => setShowFraud(f => !f)}
            className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border font-semibold transition-all ml-auto ${
              showFraud ? 'bg-orange-500 text-white border-orange-500 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-orange-400 hover:text-orange-500'}`}>
            <AlertTriangle size={13} /> Fraud Only
          </button>
        </div>
      </div>

      {!loading && <p className="text-xs text-slate-400 mb-4 font-medium">Showing {visible.length} of {complaints.length} complaints</p>}
      ...
      <div className="card overflow-x-auto">
        {loading ? (
          ...
          <table className="data-table min-w-[720px]">
            <thead>
              <tr><th>ID</th><th>Student</th><th>Title</th><th>Category</th><th>Location</th><th>Status</th><th>Date</th><th>Action</th></tr>
            </thead>
            <tbody>
              {visible.length === 0 ? (
                <tr><td colSpan={8} className="py-12 text-center text-slate-400">No complaints match the current filters.</td></tr>
              ) : visible.map(c => (
                <tr key={c.id} className="cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => setSelected(c)}>
                  ...
                  <td className="max-w-[160px]">
                    <div className="flex items-start gap-1.5">
                      {c.flaggedAsFraud && <AlertTriangle size={12} className="text-orange-500 mt-0.5 flex-shrink-0 fraud-pulse" />}
                      <span className="line-clamp-2 text-slate-700">{c.title || <span className="italic text-slate-400">Untitled</span>}</span>
                    </div>
                  </td>
                  ...
                  <td>
                    <button onClick={e => { e.stopPropagation(); setSelected(c) }} className="btn-primary py-1.5 px-3 text-xs">Manage</button>
                  </td>
                </tr>
              ))}
            </tbody>
